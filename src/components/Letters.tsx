  ...
  return (
    <div className="max-w-5xl mx-auto p-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Письма (Rasmiy Xatlar)</h1>
          <p className="text-slate-500 mt-1">Умный перевод на официальный узбекский язык</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            >
              <option value="official">Строго официально</option>
              <option value="polite">Вежливо и тепло</option>
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2 rounded-lg font-medium transition-colors cursor-pointer"
            >
              <Sparkles size={18} className={generating ? 'animate-pulse' : ''} />
              {generating ? 'Генерация...' : 'Создать письмо'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Черновик (Draft Output)</h2>
            {result && (
              <button onClick={copyToClipboard} className="flex items-center gap-1.5 text-xs font-medium text-slate-600 hover:text-slate-900 transition-colors bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-md cursor-pointer">
          ...
          {!result && !generating && (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-100 rounded-lg p-6 text-center">
              ...
              <p>Официальное письмо на узбекском появится здесь.</p>
            </div>
          )}
          ...
          {generating && (
            <div className="flex-1 flex flex-col items-center justify-center text-blue-400">
              ...
              <p className="font-medium">Перевод и структурирование...</p>
            </div>
          )}
            ...
            <div className="flex-1 overflow-auto animate-in fade-in duration-300">
              ...
              {(result.toneAnalysis || result.warnings) && (
                <div className="mt-8 bg-amber-50 rounded-lg p-4 text-sm border border-amber-100">
                  {result.toneAnalysis && (
                    <div className="mb-2">
                      <strong className="text-amber-800 block mb-1">Анализ тональности:</strong>
                      <span className="text-amber-700">{result.toneAnalysis}</span>
                    </div>
                  )}
                  {result.warnings && (
                    <div>
                      <strong className="text-amber-800 block mb-1">Предупреждения / Замечания:</strong>
